...
class MyDeque4 {
    ...
    public void increaseSize() {
        System.out.println("MyDeque4.increaseSize");
        int[] arrTap = this.arr.clone();
        this.arr = new int[this.arr.length * 2];

        int start = (this.front + 1) % arrTap.length;
        int end = (this.rear + 1) % arrTap.length;

        int idx = 1;
        for (int i = start; i != end; i = (i + 1) % arrTap.length) {
            this.arr[idx++] = arrTap[i];
        }
